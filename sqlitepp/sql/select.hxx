...
namespace sqlitepp {
    namespace sql {
        ...
        template <class CharT, class Table, class ... ColumnTypes>
        class select_t : public select_base<select_t<CharT, Table, ColumnTypes...>> {
        public:
            constexpr select_t(const table_base<Table> & table, const column_base<ColumnTypes> & ... selected_columns)
                ...
                for (auto col : std::initializer_list<column_view<CharT>>{selected_columns...}) {
        ...
        template <class CharT, class TableT>
        class select_all_t : public select_base<select_all_t<CharT, TableT>> {
        public:
        ...
        constexpr auto select(const from_table_t<TableT> & from_table, const Columns & ... columns) {
            return select_t<char, TableT, Columns...>{from_table.get_table(), columns...};
        }

        template <class TableT>
        constexpr auto select(const from_table_t<TableT> & table, all_t) {
            return select_all_t<char, TableT>{table};
        }
